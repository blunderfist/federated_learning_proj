...
##################################
    ...
    user_groups = skin_cancer_skewed(train_set, 5) # works
    user_groups = skin_cancer_skewed(train_set, args.num_users) # need to test
...
################################################################################################################################
        ...
        # for epoch in tqdm(range(2)): ##################################HARDCODED HERE GLOBAL EPOCHS################################        
...
###############################################################################################################
                ...
                cm.save_csv(os.path.join('skewed_results','local_results',f"epoch_{epoch}_fold_{int(fold)}_client_{idx}_summary"), summary = True, matrix_save = False)

                confusion_matrix_df = pd.DataFrame(cm.table)
                confusion_matrix_df.to_csv(os.path.join('skewed_results','local_results',f'epoch_{epoch}_fold_{int(fold)}_client_{idx}_confusion_matrix.csv'))
                truth_vs_preds_df = pd.DataFrame({'y_true': y_t, 'y_pred': y_p})
                truth_vs_preds_df.to_csv(os.path.join('skewed_results','local_results',f'epoch_{epoch}_fold_{int(fold)}_client_{idx}_truth_v_preds.csv'))

                cm.plot(cmap = plt.cm.Greens,
                    number_label = True, 
                    plot_lib = "matplotlib")
                plt.savefig(os.path.join('skewed_results','local_results',f"epoch_{epoch}_fold_{int(fold)}_client_{idx}_cmplot_output.png"), 
                    facecolor = 'y', 
                    bbox_inches = "tight",
                    pad_inches = 0.3, 
                    transparent = True)
...
###############################################################################################################
        ...
        # cm = pycm.ConfusionMatrix(matrix={"Class1": {"Class1": 1, "Class2":2}, "Class2": {"Class1": 0, "Class2": 5}})
        # class_label_names = {k:v for k,v in zip (range(0,len(train_set[0].classes)), train_set[0].classes)}
        print(f"{'#'*30}\n\n\nCLASS LABEL NAMES FOR PYCM\n{class_label_names}\n\n\n\n{'#'*30}")
        # cm.relabel(mapping = class_label_names)
        cm.stat(summary = True)
        to_save_as_a_file = cm.to_array()
        # with open(f'fold_{int(fold)}_client_{idx}_confusion_matris.csv', 'w') as write_file:
            # write_file.write(to_save_as_a_file)
        cm.save_csv(os.path.join('skewed_results','global_results', f"epoch_{epoch}_fold_{int(fold)}_client_{idx}_summary_global"), summary = True, matrix_save = False)

        confusion_matrix_df = pd.DataFrame(cm.table)
        confusion_matrix_df.to_csv(os.path.join('skewed_results','global_results',f'epoch_{epoch}_fold_{int(fold)}_client_{idx}_confusion_matrix_global.csv'))
        truth_vs_preds_df = pd.DataFrame({'y_true': y_t, 'y_pred': y_p})
        truth_vs_preds_df.to_csv(os.path.join('skewed_results','global_results',f'epoch_{epoch}_fold_{int(fold)}_client_{idx}_truth_v_preds_global.csv'))

        cm.plot(cmap = plt.cm.Greens,
            number_label = True, 
            plot_lib = "matplotlib")
        plt.savefig(os.path.join('skewed_results','global_results', f"epoch_{epoch}_fold_{int(fold)}_client_{idx}_cmplot_output.png"), 
            facecolor = 'y', 
            bbox_inches = "tight",
            pad_inches = 0.3, 
            transparent = True)
