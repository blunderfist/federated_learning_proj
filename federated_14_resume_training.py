...
# ======================= Save Model ======================= #
	...
	df.to_csv(os.path.join(model_path, f'{global_model._get_name()}_{args.optimizer}_{k}Folds_{args.epochs}GE_{args.local_ep}LE.csv'))
	dfc.to_csv(os.path.join(model_path, f'{global_model._get_name()}_{args.optimizer}_{k}Folds_{args.epochs}GE_{args.local_ep}LE_Clients.csv'))
	
	FINAL_WEIGHTS = average_weights(AVG_WEIGHTS)
	GLOBAL_MODEL.load_state_dict(FINAL_WEIGHTS)


	torch.save(GLOBAL_MODEL.state_dict(),(os.path.join(model_path,f'{global_model._get_name()}_{args.optimizer}_FINAL_WEIGHTS.pth')))
