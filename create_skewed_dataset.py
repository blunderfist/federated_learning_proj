...
def create_client_dirs(dir_glob, num_clients = 5):
	...
	for client in clients:
		for directory in dir_glob:
			...
			os.makedirs(os.path.join(os.getcwd(), 'skewed_dataset',f'client_{client}','val',os.path.basename(os.path.normpath(directory))))
			print("Created _______", os.path.join('skewed_dataset',f'client_{client}','val',os.path.basename(os.path.normpath(directory))))
			os.makedirs(os.path.join(os.getcwd(), 'skewed_dataset',f'client_{client}','test',os.path.basename(os.path.normpath(directory))))
			print("Created _______", os.path.join('skewed_dataset',f'client_{client}','test',os.path.basename(os.path.normpath(directory))))
	print("All directories created succesfully\n")


def train_test_split(val_size = 10, test_size = 10):
	"""
	...
	for i in selected_classes_to_split:
	...
	for i in range(len(lst_of_lst_of_files_to_split)):
		...
		train_set = len(idx_of_class) * train_size
		val_set = int(len(idx_of_class) * (val_size/100))
		test_set = int(len(idx_of_class) * (test_size/100))
		half_of_class = idx_of_class[:half_of_len_class]
		rest_of_class = idx_of_class[half_of_len_class:]
		
		for_skewed = [x for x in lst_of_lst_of_files_to_split[i] if lst_of_lst_of_files_to_split[i].index(x) in half_of_class]
		not_for_skewed = [x for x in lst_of_lst_of_files_to_split[i] if lst_of_lst_of_files_to_split[i].index(x) in rest_of_class]
